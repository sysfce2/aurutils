...
To check for AUR updates, use ~repofind -u custom~ or ~aursync -u~. Alternatively, the ~foreign~ script supports helpers which honor the ~PACMAN~ variable, e.g.:

#+BEGIN_SRC 
$ PACMAN=foreign pacaur -Syu
#+END_SRC

Or using cower:

#+BEGIN_SRC 
$ cower --ignorerepo=custom -u
#+END_SRC
