...
Alternatively, use the ~foreign~ script with helpers which honor the ~PACMAN~ variable, e.g. ~PACMAN=foreign pacaur -Syu~, or the ~--ignorerepo~ flag for ~cower~. Unlike ~repofind~, these only check updates for installed packages.
